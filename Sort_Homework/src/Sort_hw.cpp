...
//取得相差的記憶體使用量
void getMemorysize(int n) {//當前運行占用-(呼叫函式創建的arr占用-呼叫函式前的占用)=排序時的占用
...
int main() {
    ...
    testSort(quickSortMedianOfThree, "Quick Sort (Median-of-Three)", generateWorstCaseQuickSortMedianOfThree, "n^2", "O(log n)");//您可以將n^2改為nlogn會發現預測不如實際增長得快,預估實際增長速度應為n^1.7左右
    ...
    testSort(CompositeSort, "Composite Sort", randomgenerateWorstCase, "nlogn", "O(n)");//因為只有n=500使用Quick Sort (Median-of-Three)之後都是Iterative Merge Sort,所以輸入"nlogn", "O(n)"
    system("pause");
    return 0;
}
