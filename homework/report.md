...
|---------------------------------------------|----------------------------------------|
---
▲可以看到實際和預期偏差非常大，我們推測是Windows透過**分段式記憶體分配**從而導致該現象，因為如果你需要多少記憶體它就準確給你多少記憶體的話，
 會導致消耗較多系統效能，所以它會分段式調整，舉個例子:假設系統一開始設定2048KB作為一個區間，你邀求100KB，系統會直接給你2048KB，如果你邀求3000KB
 ，系統就會給你4096KB，所以你會看到記憶體實際是提高一段維持一段 （不看折線圖，從Insertion Sort看到最後的CompositeSort的總體實際記憶體占用），
 您可以調整排序呼叫的順序或註解，也會發現相同的現象，先行運行的排序總是先上升記憶體占用，然後再維持，並非我們預期的曲線，而且n在500到5000可能對系統來說不夠大，所以數值波動也不明顯
## 申論及開發報告
### 申論
